...
var jwt = require('jsonwebtoken'); // 创建一个服务器
...
http.listen('6060'); // 存放每个房间人数

var group = {}; // 监听到客户端来的ping信息 更改状态
// function heartbeat() {
//   this.isAlive = true;
// }
// function noop() {}
...
wss.on('connection', function (ws, req) {
  var token = req.headers['sec-websocket-protocol']; // buf5.toString()

  console.log(token, buf5.toString(), "江苏省");
  jwt.verify(token, buf5.toString(), function (err, authData) {
    if (err) {
      console.log(err, "失败了");
      return;
    } else {
      ...
      if (ws !== client && client.readyState === WebSocket.OPEN && ws.room === client.room) {
        ...
        if (name !== '' && m === '') {
          ...
          jwt.sign(user, name, {
            ...
            buf5 = Buffer.from(name); // const len = buf.write();
...
function getIPAdress() {
  var interfaces = os.networkInterfaces(); // console.log(interfaces)
